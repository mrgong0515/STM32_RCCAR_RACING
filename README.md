...
본 프로젝트는 대한상공회의소 서울기술교육센터 **AI 융합 로봇 SW 개발자 2기 과정** 중, ARM Cortex-M4 기반 임베디드 시스템 제어 수업의 최종 프로젝트로 진행되었습니다.

STM32F411xE MCU가 탑재된 NUCLEO-F411RE 보드를 기반으로, 수업에서 학습한 UART, I2C, SPI, PWM, ADC 등 다양한 하드웨어 자원을 통합적으로 활용하여 **무선 RC카 레이싱 시스템**을 구현했습니다.

시스템은 컨트롤러, RC카, 스타터, 피니셔 4개의 모듈로 구성되며, 무선 조종, 출발 판정, 부정 출발 감지, 결승선 통과 판정까지 하나의 레이싱 흐름으로 동작하도록 설계했습니다.
...
- 공성우
  - 역할: 도트 매트릭스 드라이버 설계, 스타터 펌웨어 개발, RC카 제작
